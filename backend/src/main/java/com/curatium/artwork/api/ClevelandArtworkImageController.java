...
import com.curatium.artwork.application.ClevelandArtworkImageService;
...
@RestController
@RequestMapping("/api/artwork-images/cleveland")
@RequiredArgsConstructor
public class ClevelandArtworkImageController {

    private static final CacheControl CACHE_CONTROL = CacheControl.maxAge(Duration.ofDays(1)).cachePublic();

    private final ClevelandArtworkImageService artworkImageService;

    @GetMapping(value = "/{accessionNumber}/{variant}", produces = MediaType.IMAGE_JPEG_VALUE)
    public ResponseEntity<byte[]> getArtworkImage(
            @PathVariable String accessionNumber,
            @PathVariable String variant,
            @RequestHeader(value = HttpHeaders.IF_NONE_MATCH, required = false) String ifNoneMatch
    ) {
        DeliveredArtworkImage image = artworkImageService.image(accessionNumber, variant);
