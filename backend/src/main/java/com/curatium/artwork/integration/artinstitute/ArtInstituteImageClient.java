...
@Component
public class ArtInstituteImageClient {

    private final HttpClient httpClient;
    private final URI iiifBaseUri;
    private final Duration readTimeout;

    public ArtInstituteImageClient(
            @Qualifier("artInstituteHttpClient") HttpClient httpClient,
            @Value("${curatium.art-institute.iiif-base-url}") String iiifBaseUrl,
            @Value("${curatium.art-institute.read-timeout}") Duration readTimeout
    ) {
        this.httpClient = httpClient;
        this.iiifBaseUri = URI.create(stripTrailingSlash(iiifBaseUrl));
        this.readTimeout = readTimeout;
    }

    public FetchedArtworkImage fetch(UUID imageId, ArtworkImageVariant variant) {
        URI uri = URI.create(iiifBaseUri + "/" + imageId + "/full/" + variant.width() + ",/0/default.jpg");
        ...
        try {
            ...
            try (InputStream body = response.body()) {
                ...
                return new FetchedArtworkImage(bytes);
            }
